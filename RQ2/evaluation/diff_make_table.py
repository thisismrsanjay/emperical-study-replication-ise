...
table_column = ['Project', "AUC", "F1", "Precision", "Recall", "MCC", "Brier", "COUNT"]
...
evaluation_name_list = ["AUC", "F1", "Precision", "Recall", "MCC", "Brier"]
...
def make_table(result_dict, rank_dict):
    ...
    for m_name in modelName:
        for delete_rate in deleted_rate_list:
            ...
            for p_name in project_name_list:
                ...
                TABLE.to_csv(path_or_buf='./tables/diff_rank_{0}_{1}_{2}.csv'.format(p_name, delete_rate, m_name),index=False, sep="&")
    ...
    TABLE.to_csv(path_or_buf='./tables/diff_rank_sum.csv', index=False, sep="&")
...
def make_table_interesting_ILA(result_dict, rank_dict):
    interest_ILA_list = ["1", "3,5", "14_1", "3,14_2", "14_1,14_2", "3,14_1,14_2", "9_2,14_1,14_2", "5,9_2,14_1,14_2"]
    ...
    for m_name in modelName:
        for ILA in interest_ILA_list:
            ...
            TABLE.to_csv(path_or_buf='./tables/diff_rank_{0}_{1}.csv'.format(m_name, ILA_name),index=False, sep="&")


def main():

    result_dict = util.load_pickle("./results/diff_result_data.pickle")
    rank_dict = util.load_pickle("./results/diff_rank_data.pickle")
