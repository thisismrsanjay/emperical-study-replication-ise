...
PLOT_YEARS_LIST = [2009, 2010, 2011, 2012, 2013]
...
RELEASE_DICT = {"AVRO": {2009: [4, 7, 9, 10], 
    ...
    "CHUKWA": {2014: [11], 2015: [12], 2016: [6]},
    "KNOX": {2013: [4, 10], 2014: [4, 11],
            2015: [1, 4, 12], 2016: [2, 4, 8, 11, 12],
            2017: [3, 8, 12], 2018: [2, 7, 12],
            2019: [7], 2020: [4, 12]}
    }
...
def make_csv(p_name):
    ...
    #if p_name=="unit_test":
    #    issue_id_type_dict, parsed_log_dict, date_repo_dict = _unit_test()

    # issue data
    # issue_id_type_dict [dict<issue id, issue type>]
    ...
    # commit log data -- commit hash and issue id
    # parsed_log_dict [dict<commit hash, dict<key name, data>>] -- key name list: author_date, commit_date, author, committer, issue_id
    log = git_reader.git_log_all_without_merge(repodir)
    parsed_log_dict = analyze_linked_commits.parse_log(log, p_name.lower())

    # commit log data -- date
    ...
    for commit_hash in parsed_log_dict.keys():
        ...
        if not tmp_year in PLOT_YEARS_LIST:
            continue
...
def plot(p_name, df):
    ...
    print(df.columns)
    df.plot(kind='line', x='date', y="Bug", ax=ax, grid=True, xticks=list(df.date), rot=90, color="red", linewidth=2)
    df.plot(kind='line', x='date', y="Improvement", ax=ax, grid=True, xticks=list(df.date), rot=90, color="blue", linewidth=2)
    df.plot(kind='line', x='date', y="New Feature", ax=ax, grid=True, xticks=list(df.date), rot=90, color="green", linewidth=2)
    df.plot(kind='line', x='date', y="Sub-task", ax=ax, grid=True, xticks=list(df.date), rot=90, color="purple", linewidth=2)
    df.plot(kind='line', x='date', y="Task", ax=ax, grid=True, xticks=list(df.date), rot=90, color="yellow", linewidth=2)
    df.plot(kind='line', x='date', y="Test", ax=ax, grid=True, xticks=list(df.date), rot=90, color="pink", linewidth=2)
    df.plot(kind='line', x='date', y="Wish", ax=ax, grid=True, xticks=list(df.date), rot=90, color="gray", linewidth=2)
    #df.plot(kind='line', x='date', y='Precision', ax=ax2, grid=True, xticks=list(df.date), rot=90, subplots=True, sharey=False, color="blue", linewidth=2)
    #df.plot(kind='line', x='date', y='Recall', ax=ax2, grid=True, xticks=list(df.date), rot=90, subplots=True, sharey=False, color="green", linewidth=2)

    for release_year in RELEASE_DICT[p_name].keys():
        for release_month in RELEASE_DICT[p_name][release_year]:
            ax.axvline(datetime(year=release_year, month=release_month, day=1), color='k', lw=1, linestyle="--")

    ax.set_xlabel("Date", fontsize=22)
    ax.set_ylabel("Number of Appearances of\nIssue Report Labels", fontsize=22)
    ax.legend(loc='upper right', fontsize=18)

    xticklabels_list = []
    pre_year = ""
    for row in list(df.date):
        if pre_year!=row.year:
            xticklabels_list.append("{0}-{1}".format(row.year, row.month))
            pre_year = row.year
        else:
            xticklabels_list.append("{0}".format(row.month))
    #ax.set_xticklabels("{0}-{1}".format(row.year, row.month) for row in list(df.date))
    ax.set_xticklabels(xticklabels_list)
    ax.tick_params(labelsize=18)

    plt.tight_layout()
    plt.savefig("./plot/{0}_for_paper.pdf".format(p_name))

def show_distribution(p_name):

    df = pd.read_csv("./tables/{0}_dist_with_time_filtering.csv".format(p_name), parse_dates=[-3])
